...
function incrementMove() {
  ...
  if (move % 2 !== 0) {
    ...
    currentImage = "cross";
    infoText.innerHTML = `It's ${players.playerOne.name}'s turn`;
  } else {
    nextPlayer = players.playerTwo.name;
    pastPlayer = players.playerOne.name;
    currentImage = "circle";
    infoText.innerHTML = `It's ${players.playerTwo.name}'s turn`;
  }
  checkForWin();
  checkForTie();
}
...
function continueGame() {
  ...
  function reset() {
    ...
    addSquareClick();
    playerHasWon = false;
    infoText.innerHTML = `It's ${nextPlayer}'s turn to start`;
  }
  ...
  function startGame() {
    ...
    form.addEventListener("submit", (event) => {
      ...
      infoText.innerHTML = `It's ${players.playerOne.name}'s turn to start`;
